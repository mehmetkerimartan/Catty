Shader "Custom/RealityHide"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _Color ("Top Color", Color) = (0.4,0.7,0.9,1)
        _DarkenedColor ("Darkened Color (when tear active)", Color) = (0.15,0.08,0.08,1)
        ...
        _TearEdgeColor ("Tear Edge Color", Color) = (0.8,0.85,1,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float smoothBlend = smoothstep(0.0, 1.0, _RealityBlend);
                half3 bottomColor = lerp(_BottomColorHeaven.rgb, _BottomColorHell.rgb, smoothBlend);
                
                /* When tear is active, blend main color to darkened version - 4th power for very late start */
                float darkenBlend = smoothBlend * smoothBlend * smoothBlend * smoothBlend;
                half3 activeColor = lerp(_Color.rgb, _DarkenedColor.rgb, darkenBlend);
                
                /* Vertical gradient fade */
                float fadeT = saturate((input.positionWS.y - _FadeEnd) / (_FadeStart - _FadeEnd));
                half3 baseColor = lerp(bottomColor, activeColor, fadeT);
                ...
                /* Tear edge glow */
                float edgeDist = dist - adjustedRadius;
                if (edgeDist < _EdgeWidth && _TearRadius >= 0.1)
                {
                    float edgeFactor = 1.0 - (edgeDist / _EdgeWidth);
                    float smoothEdge = edgeFactor * edgeFactor;
                    finalColor = lerp(finalColor, _TearEdgeColor.rgb, smoothEdge * 0.3);
                }
